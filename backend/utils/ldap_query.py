...
def get_ldap_data(email):
	...
	# Query database
	results = l.search_s("ou=Users,dc=iiit,dc=ac,dc=in", ldap.SCOPE_SUBTREE, filterstr="(mail={})".format(email))
	...
	if(len(results)!=1):
		...
		required = {'name':'cn', 'email':'mail', 'rollno':'uidNumber', 'gender':'gender'}
		for req in required:
