...
parser.add_argument("pref1", type=int, help="user id of pref 1 candidate", location="form", required=True)
parser.add_argument("pref2", type=int, help="user id of pref 2 candidate", location="form")
parser.add_argument("pref3", type=int, help="user id of pref 3 candidate", location="form")
...
class Vote(Resource):
    ...
    def post(self, election_id):
        ...
        if not (current_datetime >= voting_start_date and current_datetime <= voting_end_date):
        ...
        cand1 = Candidates.query.filter_by(election_id=election_id,user_id=args["pref1"]).first()
        cand1.pref1_counter += 1

        if args["pref2"]:
            cand2 = Candidates.query.filter_by(election_id=election_id,user_id=args["pref2"]).first()
            cand2.pref2_counter += 1

        if args["pref3"]:
            cand3 = Candidates.query.filter_by(election_id=election_id,user_id=args["pref3"]).first()
